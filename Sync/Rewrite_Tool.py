...
class RuleProcessor:
    ...
    def process_rules(self, content: str) -> Dict[str, any]:
        ...
        for line in lines:
            ...
            if 'hostname' in line.lower() and '=' in line:
                ...
                rules['host'].update(h.strip() for h in hostname.split(','))

            # 没有标签的规则
            if current_section is None:
                rules['no_section'].append(line)
            else:
                rules['sections'].setdefault(current_section, []).append(line)
    ...
    def generate_output(self, rules: Dict[str, any]) -> str:
        ...
        # 没有标签的规则放到 [REWRITE_LOCAL]
        if rules.get('no_section'):
            content.append("[REWRITE_LOCAL]")
            content.extend(rules['no_section'])
            content.append("")
        ...
        # MITM 部分
        if '[mitm]' in rules['sections'] or rules['host']:
            content.append("[MITM]")
            if '[mitm]' in rules['sections']:
                content.extend(rules['sections']['[mitm]'])
            if rules['host']:
    ...
    def update_readme(self, rules: Dict[str, any]):
        ...
        section_counts = {}
        for section, lines in rules['sections'].items():
            count = len([line for line in lines if not line.startswith('#')])

...
def download_and_merge_rules():
    ...
    for name, url in FILTER_SOURCES.items():
        try:
            ...
            rules_count = len([line for line in lines if line.strip() and not line.startswith('#')])
            source_statistics[name] = rules_count
            total_rules_before += rules_count
    ...
    # 计算去重后的规则数量
    total_rules_after = sum(len(rules) for rules in rules_dict.values())
    
    # 生成统计信息
    stats_header = f"""# 国内分流规则合集
# 更新时间：{beijing_time.strftime('%Y-%m-%d %H:%M:%S')} (北京时间)

# 规则数量统计
# 原始规则数量：{total_rules_before}
# 去重后规则数量：{total_rules_after}
# 重复规则数量：{total_rules_before - total_rules_after}

# 各规则来源统计：
{chr(10).join([f'# {name}: {count} 条' for name, count in source_statistics.items()])}
...
"""
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        for rule_type in rule_types:
            rule_count = len(rules_dict[rule_type])
            if rule_count > 0:
                f.write(f"# {rule_type}: {rule_count} 条\n")
    ...
    print(f"\n规则统计:")
    print(f"原始规则数量：{total_rules_before}")
    print(f"去重后规则数量：{total_rules_after}")
    print(f"重复规则数量：{total_rules_before - total_rules_after}")
    print("\n各来源规则数量：")
    for name, count in source_statistics.items():
        print(f"{name}: {count}")
    print("\n各类型规则数量：")
    for rule_type in rule_types:
        rule_count = len(rules_dict[rule_type])
        if rule_count > 0:
            print(f"{rule_type}: {rule_count}")
    
    return total_rules_after

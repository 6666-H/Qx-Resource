...
!(async () => {
    ...
    // 获取当前网络状态
    const currentState = {
        type: $network.v4.primaryInterface || 'unknown',
        ssid: $network.wifi?.ssid || ''
    }

    // 对比网络变化
    if (lastNetworkState.type !== currentState.type || 
        (currentState.type === 'en0' && lastNetworkState.ssid !== currentState.ssid)) {
        ...
        if (currentState.type === 'en0') {
            subtitle = `已切换至 WiFi`
            body = `当前连接: ${currentState.ssid}`
        } else if (currentState.type === 'pdp_ip0') {
            subtitle = `已切换至蜂窝数据`
            body = `网络接口: ${currentState.type}`
        } else {

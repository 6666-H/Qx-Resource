...
$.log('==================== 环境信息 ====================')
$.log('🔧 当前环境:', $.getEnv())
$.log('📱 $ 对象方法列表:', Object.getOwnPropertyNames($).join(', '))
...
try {
    ...
    $.log('🛠 $ 对象基本信息:', safeStringify(safeProps))
} catch (e) {
    $.log('打印 $ 对象时发生错误:', e)
}
...
try {
    if (typeof $network !== 'undefined') {
        $.log('📡 $network:', safeStringify($network))
    }
    if (typeof $environment !== 'undefined') {
        $.log('🌍 $environment:', safeStringify($environment))
    }
...
!(async () => {
    ...
    $.log('📱 获取到上次网络状态:', safeStringify(lastNetworkState))
    ...
    $.log('📊 当前网络状态:', safeStringify(currentState))
    $.log('📊 上次网络状态:', safeStringify(lastNetworkState))
